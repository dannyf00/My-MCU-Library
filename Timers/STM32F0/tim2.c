...
#include "tim2.h"					//we use hardware timer
...
//timer set to
void tmr2_init(uint16_t ps, uint16_t period) {
	...
	//reset isr ptr
	/*_tmr*/_isr_ptr = /*_tmr2_*/empty_handler;
...
void tmr2_act(void (*isr_ptr)(void)) {
	...
	//enable tim2 irq
	NVIC_InitStructure.NVIC_IRQChannel = TIM2_IRQn;				//tim2 irq
