...
#include "tim7.h"					//we use hardware timer

//empty handler
static void /*_tmr7_*/empty_handler(void) {
	//default tmr handler
}
...
//timer set to
void tmr7_init(uint16_t ps, uint16_t period) {
	...
	//reset isr ptr
	/*_tmr*/_isr_ptr = /*_tmr7_*/empty_handler;
...
void tmr7_act(void (*isr_ptr)(void)) {
	...
	//enable tim4 irq
	NVIC_InitStructure.NVIC_IRQChannel = TIM7_IRQn;				//tim4 irq
