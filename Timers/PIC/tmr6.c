...
void (*_tmr6_isr_ptr)(void)=empty_handler;			//tmr2 isr handler pointer
...
void tmr6_init(unsigned char scaler, unsigned char period) {
	...
	_tmr6_isr_ptr=empty_handler;			//point to default handler
...
void tmr6_act(void (*isr_ptr)(void)) {
	_tmr6_isr_ptr=isr_ptr;					//activate the isr handler
	TMR6IF = 0;									 //clear tmr2 interrupt
