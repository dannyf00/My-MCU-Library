...
void uart1_putline(unsigned char *ln)
{
	//USARTWriteString(ln);
	uart1_puts(ln);
	//USARTWriteString("\r\n");
	uart1_puts((unsigned char *)"\r\n");
}
...
unsigned char uart1_getch(void)
{
	...
	//while (UxRXIF == 0) continue;	//wait for data to be received - blocking
	//UxRXIF = 0;						//clera the flag
